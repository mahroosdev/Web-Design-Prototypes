...
// Animated counters
setTimeout(()=>{
  let c=0;const ci=setInterval(()=>{c+=37;document.getElementById('ls-customers').textContent=formatNum(Math.min(c,12400));if(c>=12400)clearInterval(ci);},20);
  let d=0;const di=setInterval(()=>{d+=89;document.getElementById('ls-deploys').textContent=formatNum(Math.min(d,89000));if(d>=89000)clearInterval(di);},20);
},300);
